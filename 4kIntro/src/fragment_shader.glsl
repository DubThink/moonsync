...
vec3 render2(in vec3 ro, in vec3 rd)
{
	...
	// fx light
	toLight=cameraCast.position-lights[2].position.xyz+vec3(cnoise(vec3(TIME*100))*.3,cnoise(vec3(TIME*100*200))*.3,cnoise(vec3(TIME*100+100))*.3);
	ldist=length(toLight);
	if(lights[2].position.w>0&&ldist<30){
		vec3 diffuse =  vec3(1.0,1.0,2.0)*mix(1/ldist*ldist,max(0,dot(cameraCast.normal,-toLight/ldist)),1)*(1+cnoise(vec3(TIME*100)));
		cool += diffuse*vec3(pointShadow(cameraCast.position.xyz, toLight, 200.0, 50));//*vec3(mod(i,1.3f),.3+mod(i,1.3f),.6+mod(i,1.3f));
	}
...
vec3 render(in vec3 ro, in vec3 rd)
{
	...
	// 	//lights[i].color.rgb *
	// 	vec3 diffuse =  lights[i].color.rgb * max(vec3(0),dot(cameraCast.normal,-toLight/ldist));//*cnoise(vec3(TIME*100));//*vec3(1-ldist/15);
	// 	cool += diffuse*vec3(pointShadow(cameraCast.position.xyz, toLight, 40.0, 80))*0.2;//*vec3(mod(i,1.3f),.3+mod(i,1.3f),.6+mod(i,1.3f));
	// 	//vec3(worldShadow(cameraCast.position.xyz, -toLight, 200.0, 200))*diffuse*0.1;
	// }
	...
	if(lights[2].position.w>0&&ldist<30){
		vec3 diffuse =  vec3(1.0,1.0,2.0)*mix(1/ldist*ldist,max(0,dot(cameraCast.normal,-toLight/ldist)),1)*(1+cnoise(vec3(TIME*100)));
		cool += diffuse*vec3(pointShadow(cameraCast.position.xyz, toLight, 200.0, 200));//*vec3(mod(i,1.3f),.3+mod(i,1.3f),.6+mod(i,1.3f));
	}
