...
const char *fragment_shader_glsl_pr =
 ...
 "{"
   ...
   "float T=sphereSDF(v,vec3(0.,10-sin(r.x/10.)*3.-sin(r.z/10.),0),4.),f=sphereSDF(v,vec3(-5.,10.-sin(r.x/10.)*3.-sin(r.z/10.)*3.,0.),2.),i=sphereSDF(v,vec3(0.,10.-sin(r.x/10.)*3.-sin(r.z/10.)*3.,5.),2.),a=sphereSDF(v,vec3(0.,10.-sin(r.x/10.)*3.-sin(r.z/10.)*3.,-5.),2.),m=sphereSDF(v,vec3(5.,10.-sin(r.x/10.)*3.-sin(r.z/10.)*3.,0.),2.),l=opSmoothUnion(T,f,sin(TIME)*2.+2.),t=opSmoothUnion(i,a,sin(TIME)*2.+2.);"
 ...
 "{"
   ...
   "return normalize(r.xyy*worldSDF(v+r.xyy*s)+r.yyx*worldSDF(v+r.yyx*s)+r.yxy*worldSDF(v+r.yxy*s)+r.xxx*worldSDF(v+r.xxx*s));"
 "}"
 "Camera getCam()"
 "{"
   "Camera v;"
   ...
   "v.up=vec3(0,1,0);"
   "v.forwards=normalize(r-v.position);"
   ...
   "float T=resolution.x/resolution.y;"
   "vec3 f=s+n.x*v.left*T+n.y*v.up;"
   "v.rayDir=normalize(f-v.position);"
   "return v;"
 "}"
 "void main()"
 "{"
   "Camera v=getCam();"
   "vec3 s=v.rayDir;"
   "raymarchResult r=worldMarch(v.position,v.rayDir);"
   "vec3 n=calcWorldNormal(r.position);"
   "color+=vec4(r.diffuse_color*max(0,dot(n,normalize(vec3(1.,1.,0.))))*vec3(1.,1.,1.),1.);"
   "color+=vec4(r.diffuse_color*max(0,dot(n,normalize(vec3(.1,-1.,.2))))*vec3(.3,.3,.5),0.);"
   "color+=vec4(r.diffuse_color*max(0,dot(n,normalize(vec3(-.9,1.,.2))))*vec3(vec3(abs(sin(TIME)),abs(sin(TIME+1)),abs(sin(TIME+2)))),0.);"
   "vec3 f=normalize(vec3(1,-1,1)),T=reflect(-f,n);"
   "float i=pow(max(0.,dot(T,v.rayDir)),200.);"
   "color.xyz+=vec3(i)*r.diffuse_color;"
 "}";
