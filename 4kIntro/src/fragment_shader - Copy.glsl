...
float pointShadow(in vec3 ro, in vec3 target, float hardness, const int MAX_STEPS) {
	vec3 rd=-normalize(ro-target);
	ro += rd*MIN_HIT_DIST*30.0; // Increasing the factor here decreases the chance of banding, but makes less accurate shadows.
	...
	for(int i = 0; i < MAX_STEPS; i++) {
		...
		if(distance(ro,target)<MIN_HIT_DIST){
			break;
		}
		if(samp < MIN_HIT_DIST) {
			return 0.0;
...
vec3 render(in vec3 ro, in vec3 rd)
{
	...
	for(int i=5;i<6;i++){
		...
		if(ldist>30)continue;
		//lights[i].color.rgb *
		vec3 diffuse =  max(vec3(0),dot(cameraCast.normal,-toLight/ldist));//vec3(1-ldist/15)*
		cool += diffuse*vec3(pointShadow(cameraCast.position.xyz, lights[i].position.xyz, 200.0, 200));
