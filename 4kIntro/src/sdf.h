...
inline float sdPlane(vec3 p, vec4 n) {
	// n must be normalized
	return dot(p, n.xyz()) + n.w;// + sin(p.x/10.0)*3.0 + sin(p.z/10.0)*3.0;
}

inline float sphereSDF( vec3 v, vec3 center, float radius)
{
...
inline float sdEnemy(vec3 v, vec3 p) {
	//float playerModel = //sdVerticalCapsule(v,2,1);
	return  sphereSDF(v, vec3(p), 1);
}
inline float sdCappedCylinder(vec3 p, vec2 h)
{
